...
void *thread(void *args) 
{
	...
	while(sampledEdgeIndex < preferredEdgeSize) {
		...
		int randomIndex = rand() % total_edges;//getRandomIndex(total_edges);
		...
		if(sampledIndexMap.find(randomIndex)->second == 0)
		{
            ...
            if ( enable_rejection )
            {
                ...
                // If yes, bypass this iteration and re-generate it again
...
int main(int argc, char *argv[])
{
    ...
    num_threads = atoi(argv[2]); //4;//

    enable_rejection = atoi(argv[3]); //0;
	...
	char *filename = argv[1];//FILE_NAME;//"fb_10.txt";////
